...
public abstract class AbstractRouting extends AsyncTask<Void, Void, ArrayList<Route>> {
    private final ArrayList<RoutingListener> _aListeners;

    static final String DIRECTIONS_API_URL = "https://maps.googleapis.com/maps/api/directions/json?";
    ...
    AbstractRouting(RoutingListener listener) {
        this._aListeners = new ArrayList<RoutingListener>();
        registerListener(listener);
    }

    private void registerListener(RoutingListener mListener) {
        if (mListener != null) {
    ...
    private void dispatchOnFailure(RouteException exception) {
        for (RoutingListener mListener : _aListeners) {
            mListener.onRoutingFailure(exception);
        }
    }

    private void dispatchOnSuccess(ArrayList<Route> route, int shortestRouteIndex) {
        for (RoutingListener mListener : _aListeners) {
